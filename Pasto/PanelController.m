...
@synthesize loadingIndicatorRunning;
@synthesize isPrefButtonPressed;
...
- (void)awakeFromNib
{
    ...
    [self.loadingIndicator setDisplayedWhenStopped:false];
    self.loadingIndicatorRunning = false;
    self.isPrefButtonPressed = false;
    self.isDetailButtonPressed = false;
...
- (IBAction)onButton:(id)sender {
    if (!self.loadingIndicatorRunning) {
        [self.loadingIndicator startAnimation:sender];
        self.loadingIndicatorRunning = true;
    }
    
    NSString * api_dev_key 			= @"a0cbcb77f01bdd1d72be3c9bcc018de3";
    NSString * api_paste_code 		= self.codeTextField.stringValue;
    if (api_paste_code.length <= 0) {
        if (self.loadingIndicatorRunning) {
            [self.loadingIndicator stopAnimation:sender];
            self.loadingIndicatorRunning = false;
        }
        //NSLog(@"%@", @"No code to paste");
        [[self.codeTextField cell] setPlaceholderString:@"Cannot paste nothing."];
        return;
    }
    NSString * api_paste_private 	= @"2"; // 0=public 1=unlisted 2=private
    NSString * api_paste_name			= @"test.php"; // name or title of your paste
    NSString * api_paste_expire_date 	= @"1D";
    NSString * api_paste_format 		= @"php";
    NSString * api_user_key 			= @"320a73977146b22332f766783ffe9df6";
    
    NSDictionary *params = @{
                             @"api_option" : @"paste",
                             @"api_user_key" : api_user_key,
                             @"api_paste_private" : api_paste_private,
                             @"api_paste_name" : [PBHelper urlEncode:api_paste_name],
                             @"api_paste_expire_date" : api_paste_expire_date,
                             @"api_paste_format" : api_paste_format,
                             @"api_dev_key" : api_dev_key,
                             @"api_paste_code" : api_paste_code
                                //[ATHelper urlEncode:api_paste_code]
                             };
    
    AFHTTPRequestOperationManager *manager = [AFHTTPRequestOperationManager manager];
    [manager setResponseSerializer:[PBHTMLSerializer serializer]];
    [manager POST:@"http://pastebin.com/api/api_post.php" parameters:params success:^(AFHTTPRequestOperation *operation, id responseObject) {
        [self.resultTextField setStringValue:(NSString *)responseObject];
        if (self.loadingIndicatorRunning) {
            [self.loadingIndicator stopAnimation:sender];
            self.loadingIndicatorRunning = false;
        }
...
- (IBAction)onCopyButton:(id)sender {
    if (self.resultTextField.stringValue.length!=0) {
        NSPasteboard * pasteBoard = [NSPasteboard generalPasteboard];
        [pasteBoard declareTypes:[NSArray arrayWithObjects:NSStringPboardType, nil] owner:nil];
        [pasteBoard setString: [self.resultTextField stringValue] forType:NSStringPboardType];
    }
}

- (IBAction)onPrefButton:(id)sender {
    self.isPrefButtonPressed = true;
}

- (IBAction)onDetailButton:(id)sender {
    self.isDetailButtonPressed = true;
}
